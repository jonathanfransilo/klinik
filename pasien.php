...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama) || empty($alamat) || empty($no_hp)) {
        ...
        if ($id) {
            ...
            $sql = "UPDATE pasien SET nama = ?, alamat = ?, no_hp = ? WHERE id = ?";
...
// Query untuk mendapatkan total data pasien
...
$query = "SELECT * FROM pasien ORDER BY nama ASC LIMIT ?, ?";
...
<table class="table table-striped table-hover">
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<td>
       <button class='btn btn-sm btn-success' onclick='showPasienForm(\"edit\", " . $row['id'] . ", \"" . addslashes($row['nama']) . "\", \"" . addslashes($row['alamat']) . "\", \"" . addslashes($row['no_hp']) . "\")'><i class='bi bi-pencil-square'></i>  Edit</button>
 ...
 <nav aria-label="Page navigation">
        <ul class="pagination justify-content-center">
            ...
            <li class="page-item <?php echo ($page <= 1) ? 'disabled' : ''; ?>">
                <a class="page-link" href="?page=pasien&hal=<?php echo $page - 1; ?>" <?php echo ($page <= 1) ? 'tabindex="-1" aria-disabled="true"' : ''; ?>>Previous</a>
            </li>
            ...
            ?>
                <li class="page-item <?php echo ($page == $i) ? 'active' : ''; ?>">
                    <a class="page-link" href="?page=pasien&hal=<?php echo $i; ?>"><?php echo $i; ?></a>
                </li>
            <?php } ?>
            
            <!-- Next button -->
            <li class="page-item <?php echo ($page >= $pages) ? 'disabled' : ''; ?>">
                <a class="page-link" href="?page=pasien&hal=<?php echo $page + 1; ?>" <?php echo ($page >= $pages) ? 'tabindex="-1" aria-disabled="true"' : ''; ?>>Next</a>
            </li>
        </ul>
    </nav>
</div>
<script>
function showPasienForm(action, id = null, nama = '', alamat = '', no_hp = '') {
